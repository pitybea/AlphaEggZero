...
def draw_win_lose_action_for_model(ab, win_lose_gt, action_gt):
    win_lose_pred = ab[1]
    print(win_lose_gt)
    print(win_lose_pred)
        
    x = range(1, len(win_lose_pred) + 1)
    l1, = plt.plot(x, win_lose_gt[::-1], c = 'green')
    l2, = plt.plot(x, win_lose_pred[::-1], c = 'red')
    l3, = plt.plot(x, [0] * len(win_lose_pred), c = 'black')

    p1 = plt.scatter(x, (np.array(action_gt) + 1)[::-1], s = 55, c = 'blue')
    p2 = plt.scatter(x, (np.array(ab[0]) + 1)[::-1], s = 15, c = 'orange')
    plt.xticks(x, x[::-1])
    ...
    for _ in range(total_train_times):
        for _ in range(round_then_train):
            ...
            while game_node.egg_leftover != 0:
                while game_node.n_visits < mcts_search_times:
                    ...
                    while round_node.egg_leftover != 0 and step <= mcts_search_depth:
                        ...
                        round_node = round_node.foward_select_PUCT(two_head_model.get_action_possibility(round_node.egg_leftover))
                        step += 1
            ...
            while game_node is not None:
                ...
                action_possibility = [game_node.play_prob[i + 1] if i + 1 in game_node.play_prob else 0.0
                                      for i in range(max_egg_per_round)]
                data_buffer.add_one_data(data, win_lose * game_node.player_label, action_possibility)
                game_node = game_node.parent
        two_head_model.train_model(data_buffer.get_data())
        frame = draw_win_lose_action_for_model(two_head_model.get_status(), win_lose_gt, action_gt)
        ani_buffer.append(frame)
    ArtistAnimation(fig, ani_buffer, interval = 600, blit = True, repeat_delay = 1000).save('learning.mp4', writer = 'ffmpeg')


if __name__ == '__main__':
    play_egg_game(egg_total = 20, max_egg_per_round = 4, buffer_size = 25, round_then_train = 21,
                  total_train_times = 100, mcts_search_times = 20, mcts_search_depth = 2)
